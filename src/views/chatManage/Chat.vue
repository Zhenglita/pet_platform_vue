         ...
         <div style="padding-bottom: 10px; border-bottom: 1px solid #ccc">在线用户<span style="font-size: 12px">（点击聊天气泡开始聊天）</span></div>
          <div style="padding: 10px 0" v-for="user in users" :key="user.username">
            ...
            <i class="el-icon-chat-dot-round" style="margin-left: 10px; font-size: 16px; cursor: pointer"
               @click="choose(user.username)"></i>

           <i class="el-icon-delete" style="margin-left: 10px; font-size: 16px; cursor: pointer" @click="del(user.username)"></i>
...
<script>
import { addAPI, delAPI } from '@/api/MessageAPI'
...
export default {
  ...
  methods: {
    async del (username) {
      const { data } = await delAPI(username)
      console.log(data)
      this.chatUser = ''
    },
    async choose (username) {
      this.chatUser = username
      const { data } = await addAPI(this.chatUser)
      console.log(data)
    },
    send () {
      if (!this.chatUser) {
    ...
    init () {
      ...
      if (typeof (WebAdminSocket) === 'undefined') {
        ...
        socket.onerror = function () {
          console.log('WebAdminSocket发生了错误')
        }
      }
    }
